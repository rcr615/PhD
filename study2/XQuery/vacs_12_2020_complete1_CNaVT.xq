for $node in db:open("CNaVT")//node[@rel="hd"]
  ...
  let $word := 
                if ($node[@sc="passive"]) then
                  ...
                  $node/@root
  let $sc := $node/@sc
  let $textid := $node/ancestor::text/[@textid]
  ...
  let $postsibs:= $node/following-sibling::node
   let $sorted := for $node in $presibs | $postsibs | $node
                    order by $node/begin
                    return $node/@rel
   return string-join(($sorted, string-join(($word, "verb"), ":"), string-join(($sc, "sc"),":"), string-join(($textid, "textid"), ":")), "|")